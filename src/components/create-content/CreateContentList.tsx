                ...
                {latestDraft && (
                  <div className="mt-4 border-t border-gray-200 pt-4">
                    <div className="flex items-center justify-between text-sm">
                      ...
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                        ${latestDraft.status === 'PENDING_FIRST_REVIEW' ? 'bg-yellow-100 text-yellow-800' :
                          latestDraft.status === 'NEEDS_REVISION' ? 'bg-red-100 text-red-800' :
                          latestDraft.status === 'PENDING_FINAL_APPROVAL' ? 'bg-blue-100 text-blue-800' :
                          latestDraft.status === 'APPROVED_FOR_PUBLISHING' ? 'bg-green-100 text-green-800' :
                          'bg-gray-100 text-gray-800'}`}
                      >
